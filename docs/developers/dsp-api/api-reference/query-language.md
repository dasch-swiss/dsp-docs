...
Here are a few rules of thumb one should keep in mind to obtain good query performance:

- Design user interaction so that you only need to request a small amount of data per query.
  In particular, avoid queries that return many dependent resources.

- Only request the data that you actually need for a specific user interaction.

- Prefer restrictive queries that include a specific literal or IRI.

- Avoid `OPTIONAL` and `UNION`.
