...
The Knora base ontology is the main built-in DSP-API ontology. Each
...
In DSP-API, each item of data belongs to some particular project. Each
project using DSP-API must define a `kb:knoraProject`, which has these
...
subclasses of `kb:Resource`, so that DSP-API can query and
...
are two possible kinds of content in a DSP-API resource: DSP-API values (see
[Values](#values)) or links to other resources (see
[Links Between Resources](#links-between-resources)). Properties that point to DSP-API values
...
property is an `xsd:string`, rather than a DSP-API value; hence it is not
versioned. A user who has modify permission on a resource (see
[Authorisation](#authorisation)) can change its label.

A resource can be marked as deleted; DSP-API does this by
...
a good storage medium for binary data such as images. Therefore, DSP-API
...
In general, each project using DSP-API must define its own subclasses of
...
DSP-API values are versioned. Existing values are not modified. Instead, a
...
:   A human-readable string representation of the value's contents,
    which is available to DSP-API's full-text search index.
...
Each DSP-API value can grant permissions (see
...
Humanities data includes many different types of dates. In DSP-API, a date
...
A DSP-API time value represents a precise moment in time in the Gregorian
...
Represents a non-DSP-API URI. Property:
...
DSP-API stores certain kinds of data outside the triplestore, in files
...
:   The name of the file as stored by DSP-API.

`internalMimeType` (1)

:   The MIME type of the file as stored by DSP-API.

`originalFilename` (0-1)

:   The original name of the file when it was uploaded to DSP-API.

`originalMimeType` (0-1)

:   The original MIME type of the file when it was uploaded to DSP-API.
...
files stored by DSP-API are also versioned. Version 1 of the DSP API
does not provide a way to retrieve a previous version of a file.
...
by DSP-API, but instead resides in an external repository
managed by some other software. The `ExternalResValue` contains the
information that DSP-API needs in order to access the
...
It is also useful to store metadata about links. For example, DSP-API
...
DSP-API uses reifications of type `kb:LinkValue` (described in
...
DSP-API allows a user to see a link if the requesting user
has permission to see the source and target resources as well as the
`kb:LinkValue`.

### Text with Standoff Markup

DSP-API is designed to be able to store text with markup, which can
...
guidelines. In DSP-API, a TEI/XML document can be stored as a file
with attached metadata, but this is not recommended, because it does not
allow DSP-API to perform searches across multiple documents.

The recommended way to store text with markup in DSP-API is to use
DSP-API's built-in support for "standoff" markup, which is stored
...
mentioning a particular person. DSP-API's standoff support is intended to
...
Associates data in some DSP-API value type with a substring in a text.
...
DSP-API resource. For example, if a repository contains resources
...
DSP-API resource describing that person. Property:

`standoffTagHasLink` (1)

:   The IRI of the resource that is referred to.

One of the design goals of the Knora base ontology is to make it easy and
...
reference in its standoff nodes, DSP-API automatically
...
DSP-API automatically updates direct links and reifications
...
`kb:SystemUser` (see [Users and Groups](#users-and-groups)). DSP-API
allows a user to see a standoff link if the user has
...
DSP-API includes a standard mapping used by the SALSAH GUI. It has the IRI
...
DSP-API's standoff is designed to make it possible to convert XML
...
resulting XML documents are converted to standoff and stored in DSP-API,
...
A future version of DSP-API will provide an API for
...
DSP-API's concept of access control is that an object (a resource or
...
:   Any user who has not logged into DSP-API is
    automatically assigned to this group.

`knora-admin:KnownUser`

:   Any user who has logged into DSP-API is automatically
...
DSP-API tries to enforce repository consistency by checking constraints
...
triplestore-dependent; DSP-API may be able to provide stricter checks with
...
DSP-API supports, and attempts to enforce, the following cardinality
constraints:
...
DSP-API requires cardinalities to be defined using blank nodes, as in the
...
Also, in DSP-API, cardinalities in the subclass can override cardinalities
...
the property. This is done using the following DSP-API-specific
properties:

`subjectClassConstraint`

:   Specifies the class that subjects of the property must belong to.
    This constraint is recommended but not required. DSP-API will attempt
...
:   If the property is an object property, specifies the class that
    ...
    have this constraint, because DSP-API relies on it to
    know what type of object to expect for the property. DSP-API will
    attempt to enforce this constraint.

`objectDatatypeConstraint`

:   If the property is a datatype property, specifies the type of
    literals that can be objects of the property. DSP-API will not attempt
...
-   If it's a resource class, all its directly defined cardinalities
    must be on DSP-API resource properties (subproperties of `kb:hasValue`
    or `kb:hasLinkTo`), and all its base classes with DSP-API IRIs must
    also be resource classes. A cardinality on `kb:resourceProperty` or
    `kb:hasValue` is forbidden. It must also have an `rdfs:label`.
-   If it's a standoff class, none of its cardinalities may be on DSP-API
    resource properties, and all its base classes with DSP-API IRIs must
...
-   If the property is a DSP-API resource property, it must have an object
...
-   Each of its base properties that has a DSP-API IRI must also be a
    DSP-API resource property.
